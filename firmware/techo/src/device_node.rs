...
//! (ULCP spec §Identities); this module is what makes that
//! true. It is an ordinary MAC + `Host` pump — the same shape as the
//! `t1000e-console` firmware on identical hardware — with the device's
//! constraints baked into [`DeviceNodePlatform`]:
//!
...
//! persisted at first boot, so [`bring_up`] is unconditional and the only
...
use umsh_ulcp_device::{MAX_CHANNEL_KEYS, MAX_DEV_PEERS};
...
/// `umsh_mac::Platform` bundle for the device node inside the device
...
/// identity; no PFS ephemerals on the device node), `MAX_DEV_PEERS` peers,
...
/// accepted unchecked).
pub type DeviceNodeMac =
    umsh_mac::Mac<DeviceNodePlatform, 1, MAX_DEV_PEERS, MAX_CHANNEL_KEYS, 4, 4, 255, 32, 4, 2>;
type DeviceNodeHandle =
    MacHandle<'static, DeviceNodePlatform, 1, MAX_DEV_PEERS, MAX_CHANNEL_KEYS, 4, 4, 255, 32, 4, 2>;
type DeviceNodeHost = Host<DeviceNodeHandle>;
type DeviceNode = LocalNode<DeviceNodeHandle>;
...
static NODE_MAC_CELL: StaticCell<AsyncRefCell<DeviceNodeMac>> = StaticCell::new();
...
/// identity is live. Built by the shared ULCP driver whenever the
...
pub use umsh_ulcp_runtime::driver::DevDomainSnapshot;

/// Latest-wins hand-off from `device_task` to the sync task. A `Signal`
/// rather than a queue: intermediate table states are irrelevant, only
/// convergence on the newest snapshot matters. On a boot that skipped
/// node bring-up (a crash-report boot) a pending snapshot just sits here
/// unconsumed.
...
pub static NODE_NAME_CHANGED: Signal<ThreadModeRawMutex, ()> = Signal::new();
...
async fn node_dev_sync_task(node: DeviceNode, mac: DeviceNodeHandle, node_key: [u8; 32]) {
    ...
    loop {
        ...
        // The gate is key equality, not mere presence. The session's
        // live identity and the one this MAC was built around can
        // legitimately differ — an installed `PROP_DEV_PRIVATE_KEY`
        // takes effect at the next boot, and `CMD_CLEAR` + `CMD_RST`
        // erases the stored key — and in every such case originating
        // traffic would be signing as an identity the device no longer
        // claims.
        let matches_live = snapshot.dev_key == Some(node_key);
        NODE_ACTIVE.store(matches_live, Ordering::Relaxed);
        // Reconcile the forwarding switch and the advertised
        // role/capabilities. Setting the MAC flag is idempotent; the
        // profile is refreshed unconditionally because the role and
        // mobility properties can move without the forwarding flag.
...
/// in [`request_beacon`], leaving the slot inert rather than blocking
/// the caller.
...
#[embassy_executor::task]
async fn node_pump_task(mut host: DeviceNodeHost) {
...
#[embassy_executor::task]
async fn node_beacon_task(node: DeviceNode, identity: SoftwareIdentity) {
...
async fn send_advertisement(
    ...
    // node is — kept current by node_dev_sync_task and
    // node_identity_profile_task — so build the payload from it rather
    // than assembling a second, drifting copy here.
...
#[embassy_executor::task]
async fn node_identity_profile_task(node: DeviceNode) {
    loop {
...
/// uses.
static IDENT_REQUEST: Signal<ThreadModeRawMutex, ()> = Signal::new();
static IDENT_RESPONSE: Signal<ThreadModeRawMutex, Option<IdentityBlob>> = Signal::new();
...
/// Whether [`bring_up`] ran this boot. Without it nothing answers
...
#[embassy_executor::task]
async fn node_identity_blob_task(node: DeviceNode, identity: SoftwareIdentity) {
    use umsh_crypto::NodeIdentity as _;
    loop {
...
/// Construct the MAC around the device identity and spawn the node
/// tasks. Call at most once. The identity is never absent — boot
/// generates and persists one when the journal is empty — so there is no
...
pub async fn bring_up(
    ...
    let load_hook: fn() = || {};
    let mac_cell: &'static AsyncRefCell<DeviceNodeMac> = NODE_MAC_CELL.init_with(|| {
    ...
    // session's live PROP_DEV_KEY against the key this MAC actually
    // holds.
    ...
    let mut host: DeviceNodeHost = Host::new(MacHandle::new(mac_cell));
    ...
    spawner.spawn(node_dev_sync_task(node.clone(), MacHandle::new(mac_cell), node_key).unwrap());
