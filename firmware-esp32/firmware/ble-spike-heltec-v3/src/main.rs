//! Version-pinning spike for the Heltec WiFi LoRa 32 V3 BLE path:
...
//! esp-rtos. The V2 spike proved this lattice on the classic ESP32; this
//! binary proves the same pinned revs drive the ESP32-S3 controller.
//!
//! GATT surface mirrors `ble-spike-techo`: a CompanionService-shaped
//! echo characteristic pair with encrypted permissions and a pairing
...
async fn advertise<'values, 'server, C: Controller>(
    ...
    let scan_len = AdStructure::encode_slice(
        &[AdStructure::CompleteLocalName(b"UMSH Heltec V3 Spike")],
        &mut scan_data,
    )?;
    Ok(peripheral
        .advertise(
...
async fn ble_app<C: Controller>(controller: C) -> ! {
    ...
    let stack = trouble_host::new(controller, &mut resources)
        .set_random_address(Address::random([0x55, 0x4d, 0x53, 0x48, 0x00, 0x03]))
