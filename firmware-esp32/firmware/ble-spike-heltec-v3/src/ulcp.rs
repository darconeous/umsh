//! Minimal ULCP responder (Phase 4, path A).
//!
//! Runs the real `umsh_ulcp_device::Session` behind the GATT frame
...
//! host-tested code the nRF devices run; only the effect execution is
...
pub use umsh_ulcp::gatt::{MAX_FRAME, Reassembler, segments};
...
type Session = umsh_ulcp_device::Session<SoftwareAes, SoftwareSha256, TX_QUEUE>;
...
const DEV_VERSION: &str = "umsh-heltec-v3-spike/0.1";
...
/// passkey), then complete the transaction with [`UlcpResponder::respond_pin`].
#[derive(Clone, Copy)]
    ...
    async fn handle_effect(
        ...
        match effect {
            Effect::ProvisionIdentity { tid } => {
                ...
                println!(
                    "ulcp: provision identity tid={tid} persisted-ok={}",
                    result.is_ok(),
                );
            ...
            Effect::SaveSnapshot { tid } => {
                ...
                println!("ulcp: save tid={tid} ok={}", result.is_ok());
                let mut emit = |bytes: &[u8]| push_frame(out, bytes);
            ...
            Effect::ClearSaved { tid } => {
                ...
                println!("ulcp: clear tid={tid} ok={}", result.is_ok());
                let mut emit = |bytes: &[u8]| push_frame(out, bytes);
            ...
            Effect::SetPairingPin { tid, pin } => {
                ...
                println!("ulcp: pin request tid={tid} present={}", pin.is_some());
            ...
            other => {
                ...
                println!("ulcp: effect dropped (no radio): {other:?}");
            }
        }
        None
    }
...
fn push_frame(out: &mut OutQueue, bytes: &[u8]) {
    ...
    if frame.extend_from_slice(bytes).is_err() || out.push(frame).is_err() {
        println!("ulcp: WARN dropped emit frame len={}", bytes.len());
    }
}
