//! The device node: a full `umsh-mac`/`umsh-node`
//! stack running on the radio itself, alongside the ULCP session.
//!
//! Port of `techo/src/device_node.rs` to the ESP32-S3
...
//! persisted at first boot, so [`bring_up`] is unconditional and the
//! only question is whether the node is *transmitting*. That is
//! configuration — the PHY enable state and the forwarding switch —
//! plus the `NODE_ACTIVE` gate, which closes while a factory reset is in
//! flight.
...
use umsh_ulcp_runtime::duty_gate::DutyGatedRadio;
...
async fn node_dev_sync_task(node: DeviceNode, mac: DeviceNodeHandle, node_key: [u8; 32]) {
    ...
    loop {
        ...
        // role/capabilities; both are idempotent, and the role and
...
/// uses.
static IDENT_REQUEST: Signal<CriticalSectionRawMutex, ()> = Signal::new();
static IDENT_RESPONSE: Signal<CriticalSectionRawMutex, Option<IdentityBlob>> = Signal::new();
