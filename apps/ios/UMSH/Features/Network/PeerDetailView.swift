            ...
            if !peer.isSaved, !peer.isUlcpDevice, isStoredLocally,
               actions.promoteToSaved != nil {
                Section {
                    ...
                    Text("This node was heard over the air but is not saved. Saving records it on this phone so it appears in Network.")
                }
            }
        ...
        if isStoredLocally, !peer.isUlcpDevice, peer.isSaved,
           actions.demoteToTransient != nil || actions.deletePeer != nil {
            Section {
                ...
                Text(hasConversation
                     ? "Removing keeps the conversation; the node leaves the Network list but stays findable through search."
