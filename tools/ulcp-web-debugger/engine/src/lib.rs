...
pub fn web_sim_config() -> umsh_ulcp_simdev::SessionConfig {
    ...
    SessionConfig {
        ...
        battery_diagnostics: umsh_ulcp::battery_diagnostics::Fields::DIAGNOSTICS,
...
const PROPERTY_SPECS: &[PropertySpec] = &[
    ...
    spec(
        ...
        "integer",
        Some("Hz"),
        Some(umsh_ulcp::ids::cap::PHY_LORA),
    ),
    spec(
        prop::PHY_LORA_SF,
        "Radio",
    ...
    spec(
        ...
        "none",
        None,
        Some(umsh_ulcp::ids::cap::PHY_DUTY_LIMIT),
    ),
    spec(
        prop::PHY_DUTY_LIMIT,
        "Radio",
    ...
    spec(
        ...
        "boolean",
        None,
        Some(umsh_ulcp::ids::cap::HOST_FILTER),
    ),
    spec(
        prop::SAVED,
        "Device",
    ...
    spec(
        ...
        "none",
        None,
        Some(umsh_ulcp::ids::cap::DEV_IDENTITY),
    ),
    spec(
    ...
    spec(
        ...
        "text",
        None,
        Some(umsh_ulcp::ids::cap::DEV_NAME),
    ),
    spec(
        ...
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_CURRENT),
        None,
    ),
    spec(
        ...
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_REMAINING_CAPACITY),
        None,
    ),
    spec(
        ...
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_FULL_CAPACITY),
        None,
    ),
    spec(
        ...
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_DESIGN_CAPACITY),
        None,
    ),
    spec(
        prop::BATTERY_EXT_POWER_PRESENT,
        "Device",
        "Battery ext power present",
        true,
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_EXT_POWER_PRESENT),
        None,
    ),
    spec(
        ...
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_PRESENT),
        None,
    ),
    spec(
        ...
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_GAUGE_FULL),
        None,
    ),
    spec(
        prop::BATTERY_GAUGE_INITIALIZED,
        "Device",
        "Battery gauge initialized",
        true,
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_GAUGE_INITIALIZED),
        None,
    ),
    spec(
        prop::BATTERY_GAUGE_SMOOTHING,
        "Device",
        "Battery gauge smoothing",
        true,
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_GAUGE_SMOOTHING),
        None,
    ),
    spec(
        prop::BATTERY_CHARGE_VOLTAGE_REQUEST,
        "Device",
        "Battery charge voltage request",
        true,
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_CHARGE_VOLTAGE_REQUEST),
        None,
    ),
    spec(
        prop::BATTERY_GAUGE_FORMAT,
        "Device",
        "Battery gauge format",
        true,
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_GAUGE_FORMAT),
        None,
    ),
    spec(
        ...
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_GAUGE_STATUS),
        None,
    ),
    spec(
        prop::BATTERY_GAUGE_OPERATION_STATUS,
        "Device",
        "Battery gauge operation status",
        true,
        false,
        "none",
        umsh_ulcp::battery_diagnostics::unit(prop::BATTERY_GAUGE_OPERATION_STATUS),
        None,
    ),
    spec(
        prop::BATTERY,
    ...
    spec(
        ...
        "none",
        None,
        Some(umsh_ulcp::ids::cap::HOST_KEYS),
    ),
    spec(
        prop::HOST_PEER_KEYS,
        "Host",
    ...
    spec(
        prop::HOST_AUTO_ACK,
        "Host",
        "Delegate acknowledgements to the device",
        true,
        true,
        "boolean",
        None,
        Some(umsh_ulcp::ids::cap::HOST_AUTO_ACK),
    ),
    spec(
        ...
        "none",
        Some("frames"),
        Some(umsh_ulcp::ids::cap::HOST_RX_QUEUE),
    ),
    spec(
...
impl DebuggerEngine {
    ...
    pub fn prop_set(&mut self, key: u32, value: &[u8]) -> Result<(), &'static str> {
        ...
        let len = frame::prop_set(&mut buf, tid, key, value)
            .map_err(|_| "property value does not fit in a ULCP frame")?;
        self.queue_frame(&buf[..len]);
        Ok(())
    }
    ...
    pub fn prop_insert(&mut self, key: u32, value: &[u8]) -> Result<(), &'static str> {
        ...
        let len = frame::prop_insert(&mut buf, tid, key, value)
            .map_err(|_| "property item does not fit in a ULCP frame")?;
        self.queue_frame(&buf[..len]);
        Ok(())
    }
    ...
    pub fn prop_remove(&mut self, key: u32, value: &[u8]) -> Result<(), &'static str> {
        ...
        let len = frame::prop_remove(&mut buf, tid, key, value)
            .map_err(|_| "property selector does not fit in a ULCP frame")?;
        self.queue_frame(&buf[..len]);
        Ok(())
    }
    ...
    fn pump_gets(&mut self) {
        while let Some(key) = self.queued_gets.pop_front() {
            match self.prop_get(key) {
                Ok(()) => {}
                Err("all ULCP transaction identifiers are busy") => {
    ...
    fn queue_frame(&mut self, bytes: &[u8]) {
        self.trace(Direction::HostToDevice, bytes);
        match self.transport {
            Transport::SerialHdlc => {
    ...
    fn ingest_frame(&mut self, bytes: &[u8]) {
        self.trace(Direction::DeviceToHost, bytes);
        let Ok(frame) = Frame::parse(bytes) else {
            self.protocol_error("malformed ULCP frame".into());
            return;
        };
        let Some(command) = frame.command() else {
            self.protocol_error(format!("unknown ULCP command {}", frame.cmd));
            return;
        };
        ...
        if payload.key == prop::LAST_STATUS && !unsolicited {
            if let Some(command) = self.pending_commands[tid as usize].take() {
                let (status, success) = match pui::decode(payload.value) {
                    Ok((code, _)) => (format!("{:?}", umsh_ulcp::Status(code)), code == 0),
            ...
            && pending.is_some_and(|pending| pending.key != prop::LAST_STATUS)
        {
            ...
            let status = pui::decode(payload.value)
                .map(|(code, _)| format!("{:?}", umsh_ulcp::Status(code)))
        ...
        if payload.key == prop::LAST_STATUS && unsolicited && self.attach.announced {
            if let Ok((code, _)) = pui::decode(payload.value) {
                let status = umsh_ulcp::Status(code);
                if status.is_reset() {
                    ...
                    } else if status == umsh_ulcp::Status::RESET_RESTORED {
    ...
    fn capture_attach_property(&mut self, key: u32, value: &[u8]) {
        match key {
            ...
            prop::DEV_VERSION => {
                ...
                self.attach.dev_version = Some(String::from_utf8_lossy(value).into_owned());
            }
            prop::CAPS => {
                let mut rest = value;
    ...
    fn trace(&mut self, direction: Direction, bytes: &[u8]) {
        let redacted = direction == Direction::HostToDevice && secret_bearing_write(bytes);
...
fn decode_property(key: u32, value: &[u8]) -> Option<DecodedValue> {
    let decoded = match key {
        prop::LAST_STATUS => {
            ...
            let status = format!("{:?}", umsh_ulcp::Status(code));
        ...
        prop::INTERFACE_TYPE => {
            ...
            let display = if interface == umsh_ulcp::ids::INTERFACE_TYPE {
        ...
        key if umsh_ulcp::battery_diagnostics::index(key).is_some() => {
            use umsh_ulcp::battery_diagnostics::{self, Value};
            let display = match Value::decode(key, value).ok()? {
                None => "unavailable".into(),
                Some(value) => match battery_diagnostics::unit(key) {
                    Some(unit) => format!("{value} {unit}"),
                    None => value.to_string(),
                },
            };
            ("battery_diagnostic", display)
        }
        prop::BATTERY => {
            let status = umsh_ulcp::battery::BatteryStatus::decode(value).ok()?;
...
mod wasm {
    ...
    #[cfg(feature = "sim-device")]
    #[wasm_bindgen(js_name = SimulatedDevice)]
    pub struct WebSimulatedDevice(SimulatedDevice);
    ...
    #[cfg(feature = "sim-device")]
    #[wasm_bindgen(js_class = SimulatedDevice)]
...
mod tests {
    ...
    fn ingest_serial_frame(engine: &mut DebuggerEngine, ulcp_frame: &[u8]) {
        let mut wire = vec![0; hdlc::max_encoded_len(ulcp_frame.len())];
        let wire_len = hdlc::encode_frame(ulcp_frame, &mut wire).unwrap();
        engine.ingest(&wire[..wire_len]);
    }
    ...
    fn status_response_is_attributed_to_the_requested_property() {
        ...
        let len = frame::last_status(
            &mut response,
            request.header.tid(),
            umsh_ulcp::Status::INVALID_STATE,
        )
        .unwrap();
    ...
    #[test]
    fn lora_property_choices_match_device_constraints() {
    ...
    fn stream_receive_decodes_metadata_and_packet_header() {
        let mut ulcp_frame = [0; FRAME_CAPACITY];
    ...
    fn host_engine_attaches_through_real_simulated_session() {
        ...
        while let Some(wire) = engine.take_outbound() {
            device.ingest(&wire, 1_000).unwrap();
            while let Some(response) = device.take_outbound() {
        ...
        assert!(events.iter().any(|event| matches!(
            ...
            Event::Attached {
                ...
                phy_mtu: 255,
                ..
            } if dev_version == "umsh-web-sim/0.1"
        )));
        assert!(events.iter().any(|event| matches!(
            event,
            Event::Property {
