# UMSH ULCP web debugger

This directory contains the first working slice of the static ULCP
debugger described in [`docs/ulcp-web-debugger-design.md`](../../docs/ulcp-web-debugger-design.md):
...
`umsh-ulcp-device::Session` in WebAssembly with deterministic RAM-backed
...
```sh
make web-debugger
python3 -m http.server --directory tools/ulcp-web-debugger/www 8000
```
...
serial session detaches any attached host. The debugger performs only
...
```sh
node --test tools/ulcp-web-debugger/www/packet-capture.test.mjs
```

The simulator is enabled by the engine crate's default `sim-device` feature. Build
