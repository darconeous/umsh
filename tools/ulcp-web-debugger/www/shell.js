...
async function start() {
  ...
  bindUi();
  try {
    const wasm = await import("./pkg/umsh_ulcp_web_engine.js");
    await wasm.default();
    SimulatedDevice = wasm.SimulatedDevice;
...
function bindUi() {
  ...
  elements.connectSimulator.addEventListener("click", () => connect(new SimulatedLink(SimulatedDevice), "serial_hdlc", false));
...
async function connect(link, transport, displacesHost = true) {
  if (!session) return;
  if (displacesHost && !confirm("Attaching resets the ULCP session and displaces any host already using this radio. Continue?")) return;
...
function handleEvent(event) {
  switch (event.type) {
    ...
    case "attached":
      ...
      elements.deviceVersion.textContent = event.dev_version || "(empty)";
...
const commandConfirmations = {
  ...
  reset: "Reset the device's protocol state and reload its saved configuration? The active session will be resynchronized.",
...
function updatePropertyAvailability() {
  for (const { row, spec, value, change } of propertyRowByKey.values()) {
    ...
    if (!supported) value.textContent = "Not supported by this device";
...
function captureMetricLabels(event) {
  ...
  return [
    ...
    meta?.acknowledged && "Device acknowledged",
  ].filter(Boolean);
}
...
function traceRow(entry) {
  ...
  const direction = entry.direction === "host_to_device" ? "HOST → DEVICE" : "DEVICE → HOST";
...
function exportTrace() {
  const blob = new Blob([JSON.stringify({ format: "umsh-ulcp-trace", version: 1, trace }, null, 2)], { type: "application/json" });
  downloadBlob(blob, `umsh-ulcp-${captureTimestamp()}.json`);
}
