...
crates/
  ...
  umsh-app-ulcp-cli/   # app-specific policy: button-event ↔ action
                            #   mapping, CLI commands, MAC integration
firmware/
  t1000e-console/     # binary glue
```
...
### `umsh-app-ulcp-cli` (firmware logic, `no_std + alloc`)
...
### `firmware/t1000e-console` (binary)
...
| ~~Button held at boot~~ | — | **Removed 2026-07-16.** The UX guidelines reserve every button gesture for user actions (`docs/ux/src/hardware/t1000e.md`), and the powered-sleep wake path reboots via `SYSRESETREQ` while the wake press is often still held — a boot-time button check would bounce that wake straight into serial DFU. The 1200-baud touch, CLI commands, rescue escape, and the hardware double-tap circuit remain the DFU paths. Do not reintroduce button-gesture DFU. |
...
`memory.x`, `scripts/flash.py`, and the `Makefile` `flash-t1000e-console`
...
CLI — add them as app-specific commands in `umsh-app-ulcp-cli`.
...
- `cargo build -p firmware-t1000e-console --release` (target pinned in
