...
The spec requires it. `docs/protocol/src/ulcp.md` §Identities: the
...
presence or capabilities of the radio." `ulcp-full.md`
...
`DEVICE_CTL`) remains the single mux point for radio clients; **nothing added
here may assume `Session` is the radio's only client**"). This plan walks
through that door.

## Current state

- `device_task` wires `Session` directly to `RADIO_CH`
...
- The full node stack is already proven on both boards in other images:
  `t1000e-console` runs `MacHandle` + `LocalNode`/`Host` over the same
...
~~~
                       ┌──────────────────────────────┐
 INPUT_CH ─► device_task ─┤ Session (host domain,        │
 ...
 button ──► beacon req │ device node:                 │   ┌──────────┐   real
             ...
             node API  │  (device identity, dev       │   │  task    │    + DEVICE_CTL
                       │   channels, dev peers)       │   └──────────┘    device_runner
                       └──────────────────────────────┘         ▲
                                        virtual Channels B ─────┘
~~~
...
`cargo test -p umsh-ulcp-device` plus the affected crates.
...
Acceptance: provision a device channel with `umsh-ulcpctl`, confirm the
...
| `device_task` future pool | 29.5 KiB | 21.4 KiB | shared snapshot scratch + slim persist |
...
`umsh-ulcpctl duty` reports/sets the limit; `rf-dev-unicast` in
`ulcp_hw_validate` drives pairwise-sealed unicast at the device
...
- `docs/protocol/src/ulcp.md` §Identities, §Hybrid Use
- `docs/protocol/src/ulcp-full.md` §Identity Model, §State
