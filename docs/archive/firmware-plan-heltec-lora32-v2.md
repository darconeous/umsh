...
- Shared crates (`umsh-hal`, `umsh-mac`, `umsh-radio-loraphy`,
  `umsh-ulcp-device`, `umsh-ux-tracker`, …) come in as path deps
...
- **Sync-word mapping check:** `meshcore_us_params` carries the
  ...
  single-byte form (0x12). Verify what `device_runner`/`runner` hand to
...
- **Refactor (root workspace):** the NCP binary logic lives in
  ...
  (working name `umsh-ulcp-device-fw`), leaving the nRF main as
