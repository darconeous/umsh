...
firmware/sensecap-solar-console/ bringup + CLI binary
firmware/sensecap-solar/ the real deliverable (later phase)
```
...
and the Makefile targets (`build-/flash-sensecap-solar-console`,
...
- `firmware/sensecap-solar-console/` — binary, started as a
  stripped copy of `wio-tracker-l1-console` (no OLED — this
...
Bring the binary up to full `wio-tracker-l1-console` parity:
`Mac<SensecapSolarPlatform>` coordinator, persistent identity +
counter journal in the standard NV region, `umsh-app-ulcp-cli`
...
`firmware/sensecap-solar/` joining the shared NCP
main.rs shape (per the T-Echo/T1000-E unification — this is the
second test, after T1000-E, that the `DeviceEnv` hook set is actually
...
```
make build-sensecap-solar-console
make flash-sensecap-solar-console
```
