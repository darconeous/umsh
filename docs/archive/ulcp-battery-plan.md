...
`crates/umsh-ulcp/src/ids.rs` currently ends the device property block
at `PROP_DEV_NAME = 68` and the full capability block at
`CAP_DEV_NAME = 38`. `crates/umsh-ulcp/src/describe.rs` supplies the
...
`umsh-ulcp`, not in a board crate or the allocating host adapter,
...
`crates/umsh-ulcp-device/src/session.rs` is the authoritative property
...
| Host surface | Typed `battery_status()` accessor; **not** part of `DeviceSync` | Telemetry, not configuration; keeps a measurement hiccup from failing `sync` and the attach workflows built on it |
| Initial update behavior | Query-only | Compliant with the spec's `MAY`; no new event source in `device_task` before a consumer exists |
...
Update `crates/umsh-ulcp-device/src/session.rs`:
...
- `umsh/tests/ulcp_full_protocol.rs` services the effect with all
...
- Add `battery_status() -> Result<Option<BatteryStatus>,
  UlcpError>`: `Ok(None)` when `CAP_BATTERY` is absent;
...
- Do **not** add battery to `DeviceSync` or fetch it during `sync`.
- Leave `PropEvent` generic; a later unsolicited-update increment needs no
  transport changes.

Update user-facing tools:

- Teach `umsh-ulcpctl` the `BATTERY` capability name and have `info`
...
- Include a battery read in `ulcp_hw_validate`'s dump and add a
...
~~~sh
cargo test -p umsh-ulcp
cargo test -p umsh-ulcp-device
cargo test -p umsh --test ulcp_full_protocol
cargo test -p umsh-ulcp-web-engine
git diff --check
...
make build-techo
make build-t1000e
cd firmware/t1000e && cargo build --release --features no-ble
~~~
