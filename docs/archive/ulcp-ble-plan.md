...
The serial `ulcp_link_soak` hardware run completed 180/180 property
...
new `firmware-t1000e` target shares the proven session,
...
The compact diagnostic `PROP_DEV_VERSION` string currently carries
...
[Full Companion Radio Protocol](protocol/src/ulcp-full.md). The full
...
* `crates/umsh-ulcp` currently defines the minimal command grammar,
...
* `crates/umsh-ulcp-device::Session` is the framing-free, host-tested NCP
...
* `umsh::ulcp::UlcpDevice` supplies the desktop host's minimal
...
* `firmware/techo/src/main.rs` is shared by the T-Echo and
...
Extend `UlcpDevice` with correlated command helpers and response handling
...
`UlcpDevice` gained `insert_prop_item`/`remove_prop_item` (digest-returning,
...
Follow-up (2026-07-16): MAC-owner hosts (desktop_chat, cli_ulcp)
...
matching). Design it once in the `umsh::ulcp` workflow API
...
API addition: public `UlcpDevice::reset()` (CMD_RST + reset
...
Implements the [Companion Radio over BLE](protocol/src/ulcp-ble.md)
...
on the T-Echo NCP firmware and the host-side `UlcpDevice` driver.
...
- The SAR transport lives in `umsh-ulcp::gatt` as a
  service-agnostic module (like `hdlc`), not welded to the companion
  service.
- UUID slots `0x0100+` stay unallocated.
- The radio runner (`RADIO_CH` + `DEVICE_CTL`) remains the single mux
...
────                                    ──────────────────────────────────
umsh::ulcp                   device_task: Session + Emitter
  UlcpDevice<L: FrameLink>*           (framing removed from this task)*
    SerialFrameLink<IO>  (HDLC)*        usb_in_task: HDLC decode*   ble_task*
    BleFrameLink (btleplug + SAR)*      output_task: HDLC encode*   (SAR both
                                                                     directions,
umsh-ulcp (wire crate, no_std)     pairing mode, adv control)
...
- Pairing mode is the gate: overall, `pairing_enabled = pairing_mode &&
  ...
  only where a PIN is configured, which is where it has something to
  protect: LESC Passkey Entry leaks passkey bits per failed attempt, and
  once the failure counter trips this is what stops the probing. Under
  Just Works there is no passkey to leak, and applying the same counter
...
- `UlcpDevice<L: FrameLink>`: `ingest(&[u8])` becomes
...
- `open_serial` keeps its exact signature and the `serial-radio`
  feature; it returns
  `UlcpDevice<SerialFrameLink<tokio_serial::SerialStream>>`.
...
- **Discovery/connect**: scan for `gatt::SERVICE_UUID`; selector is
  ...
  `UlcpDevice::open_ble(selector, config)` mirrors `open_serial`.
...
to the transport edges so `device_task` deals only in companion frames:
...
- `output_task` HDLC-encodes (keeps `WIRE_MAX`) and chunks to 64-byte
  USB packets.
- `device_task` keeps the session-reset-on-attach and decoder-free logic.
...
- **`PROP_BLE_PAIRING_PIN` (4864)** — deferred completion, mirroring
  ...
  - Host side: `UlcpDevice::finish_prop_transaction` currently
...
**Exit criteria.** `ulcp_probe --ble` and `desktop_chat --ble`
...
- Supervision-timeout behavior: walk out of range mid-session; clean
  detach + re-attach; verify `device_task` arbitration state recovers.
