...
simulated device entirely in the browser, because the real session engine
...
* Work with **zero hardware** via a browser-resident simulated device
  (the real `umsh-ulcp-device` `Session` behind a virtual link),
...
write the device does not support. The debugger defaults to conservative
...
segmented by the device against the real negotiated MTU and reassemble
...
│  • HDLC encode/decode      (umsh-ulcp)    │
│  • SAR segment/reassemble  (umsh-ulcp)    │
│  • frame parse + describe  (umsh-ulcp)    │
│  • TID allocation, transaction correlation,    │
│    attach handshake, sync procedure            │
│  • MAC header / meta decode (umsh-core, meta)  │
│  • optional: simulated device (umsh-ulcp-device)│
└────────────────────────────────────────────────┘
```
...
**The Rust core is sans-IO,** in the style of the device-side `Session`:
...
deliberately **not** a port of the tokio `UlcpDevice` driver: that
...
The engine crate depends on `umsh-ulcp` (frame, hdlc, gatt, ids,
meta, items), `umsh-core` (PacketHeader, PublicKey base58 display),
and optionally `umsh-ulcp-device` + `umsh-crypto/software-crypto`
behind a `sim-device` feature. All logic is target-independent; only the
...
* **Testing:** engine unit tests on the host (transaction
  ...
  `Session` in-process, mirroring `ulcp_full_protocol.rs`;
