...
When forward secrecy is needed, UMSH provides [PFS sessions](mac-commands.md#pfs-session-request-6) — a lightweight handshake where both nodes exchange ephemeral Ed25519 keypairs and communicate using session-specific keys for an agreed duration. PFS sessions add no per-packet overhead once established, and ephemeral private keys are erased when the session ends. This provides perfect forward secrecy without the fragility of continuous ratcheting.
