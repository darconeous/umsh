...
| Destination hint (2 bytes) | Unicast | Stable per-identity; enables tracking a node's correspondents over time |
| Source hint (2 bytes) | Unicast, unencrypted multicast | Stable per-identity; enables tracking a node's activity over time |
