...
> [!NOTE]
> This chapter has been superseded and is retained for reference. Its
...
* **The device identity** — a node belonging to the device itself,
  ...
  the device and is never readable through this protocol.
...
on the host's behalf. All pairwise key material the device uses for the host
identity is derived by the host and explicitly provisioned per peer (see
(#prop-host-peer-keys)). The device identity is different: the device holds
...
tables read back in digest form only (see (#provisioning-security)), so a
...
1. `CMD_PROP_GET` for `PROP_LAST_STATUS`. If it returns a reset code (see
   [Reset Codes](ulcp-minimal.md#reset-codes)), the device has
...
2. `CMD_PROP_GET` for `PROP_HOST_KEY`. An empty value is the ordinary
   ...
   *other* value means another host has taken the radio over since this
   ...
   (see (#host-replacement)) before doing anything else.
...
4. Establish the complete host domain (see (#host-domain)): write
...
* **Post-reset values come from the snapshot.** `CMD_RST` reverts
  ...
  followed by `CMD_RST`. A host that implements only the minimal protocol
  and expects documented defaults after `CMD_RST` will find the PHY
  already configured and enabled on a radio that was provisioned for
  autonomous operation; such a host still works if it explicitly sets the
...
* `PROP_SAVED` **MUST** report a fallback and an unreadable snapshot
  ...
  (#prop-saved)). Devices with a local indicator **SHOULD** signal it
...
configuration to the contents of the saved snapshot (see (#saved-state)).
...
(#prop-dev-private-key)) would otherwise bring the radio up advertising
...
(see (#identity-model)). The public key is also emitted as the success
...
the entire host domain, as specified in (#host-replacement). Setting it to
...
specified in (#ack-delegation). When false, the device never transmits on the
...
full-protocol device in its factory state. As soon as any filter (implicit or
...
(see [Status Codes](ulcp-minimal.md#status-codes)):
...
(see [Reset Codes](ulcp-minimal.md#reset-codes)):
...
(see [Capabilities](ulcp-minimal.md#capabilities)):
