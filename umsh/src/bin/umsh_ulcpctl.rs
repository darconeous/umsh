//! ULCP device management: inspection, provisioning, device
//! identity, persistence, and pairing-PIN configuration for a device
//! over USB serial or BLE.
//!
//! Attaches with the full-protocol non-resetting handshake
//! ([`UlcpDevice::attach_existing`]), so pointing this tool at an
...
async fn provision<L: FrameLink>(
    ...
    match sync.ownership {
        ...
        HostOwnership::OtherHost(other) if !force => {
            return Err(format!(
                "the device is provisioned for another host ({}); re-run with --force to \
    ...
    // in full on every attach. `--no-save` is accepted and inert here.
    let _ = no_save;
    println!("note: host provisioning is live only — it is re-established on every attach");
    Ok(())
}
...
async fn dispatch<L: FrameLink>(
    ...
    // Every command here administers a device rather than tethering to
    // it — this tool is not anybody's radio — so attach administratively
    // and let the handle refuse host-domain writes. `provision` is the
    // one exception: it exists to establish a host domain, so it says so
    // by attaching as a tethered host.
    let tethered = matches!(invocation.command, Command::Provision { .. });
    let mut radio = if tethered {
...
/// listings and mutation reports print whatever digest the device quotes.
...
mod tests {
    ...
    fn ble_selector_forms_are_unambiguous() {
        ...
        let next_arg = parse_invocation(&args(&["--ble", "UMSH T-Echo", "info"])).unwrap();
        assert_eq!(
            next_arg.transport,
            Transport::Ble(Some("UMSH T-Echo".into()))
        );

        let equals = parse_invocation(&args(&["--ble=UMSH T-1000E", "info"])).unwrap();
        assert_eq!(
            equals.transport,
            Transport::Ble(Some("UMSH T-1000E".into()))
        );
