//! Repeatable ULCP-link control-plane soak and latency measurement.
//!
//! This intentionally performs no LoRa transmissions. It repeatedly reads a
//! stable device property, exercising the complete request/response transport and
...
impl Options {
    ...
    fn usage() -> String {
        "usage: ulcp_link_soak <serial-port> | --ble [selector] [--duration=SECONDS] [--interval-ms=MILLISECONDS] [--segment-payload=1..511]".to_owned()
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if options.transport == "--ble" {
        ...
        {
            let link_config = umsh::ulcp::BleFrameLinkConfig {
                segment_payload: options.segment_payload,
                ..Default::default()
            };
            let radio = UlcpDevice::open_ble_with_link_config(
    ...
    {
        let radio = UlcpDevice::open_serial(&options.transport, 115_200, config).await?;
...
async fn run<L: FrameLink>(
    ...
    println!("dev_version={}", radio.dev_version());
    ...
    while started.elapsed() < options.duration {
        ...
        radio.get_prop(prop::DEV_VERSION).await?;
