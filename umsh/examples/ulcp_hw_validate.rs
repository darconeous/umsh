//! Increment-9 hardware validation for the full ULCP
//! protocol: drives the same workflow the adapter-free integration
//! tests prove in-process (`umsh/tests/ulcp_full_protocol.rs`)
...
async fn open(
    ...
    let mut radio = UlcpDevice::attach_existing(SerialFrameLink::new(stream), config()).await?;
    println!(
        "attached in {:?}: device={} boot_status={:?}",
        started.elapsed(),
        radio.dev_version(),
        radio.boot_status()
    );
...
async fn phase_a(port: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    expect(sync.saved == Some(SavedSnapshot::Current), "snapshot saved")?;
...
async fn phase_b(port: &str, expected_dev_key: [u8; 32]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Current, not merely saved: a boot that fell back to an older
    // generation would still be "saved" and is exactly what this phase
    // exists to catch.
    expect(
        sync.saved == Some(SavedSnapshot::Current),
        "snapshot still saved, newest generation",
    )?;
    ...
    expect(
        !report.host_replaced && report.peers_removed == 0,
        "reattach reconcile found no differences",
    )?;
...
async fn phase_c(port: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sync = radio.sync(Some(&HOST_KEY)).await?;
    print_sync(&sync);
    expect(sync.saved == Some(SavedSnapshot::None), "snapshot erased")?;
    expect(
...
async fn phase_d(port: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    expect(
        sync.saved == Some(SavedSnapshot::None),
        "factory: nothing saved",
    )?;
...
async fn rf_peer(
    ...
    let mut radio = UlcpDevice::new(SerialFrameLink::new(stream), config()).await?;
    println!("peer device={} base counter={base}", radio.dev_version());
    if let Some(power) = power_dbm {
    ...
    // The peer device's CMD_RST restored whatever host provisioning its
...
async fn rf_dev_multicast(
    ...
    let mut radio = UlcpDevice::new(SerialFrameLink::new(stream), config()).await?;
    println!(
        "peer device={} base counter={base} count={count}",
        radio.dev_version()
    );
...
async fn rf_advert_request(
    ...
    let mut radio = UlcpDevice::new(SerialFrameLink::new(stream), config()).await?;
    println!(
        "peer device={} counter={counter} nonce={nonce:?}",
        radio.dev_version()
    );
...
async fn ble_sync(selector: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let started = Instant::now();
    let mut radio = UlcpDevice::attach_existing(link, config()).await?;
    println!(
        "BLE attached in {:?}: device={} boot_status={:?}",
        started.elapsed(),
        radio.dev_version(),
        radio.boot_status()
    );
    ...
    expect(
        sync.saved.is_some_and(SavedSnapshot::is_saved),
...
async fn print_battery<L: FrameLink>(radio: &mut UlcpDevice<L>) {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.get(1).map(String::as_str) {
        ...
        _ => {
            eprintln!(
                "usage: ulcp_hw_validate phase-a|phase-c|phase-d <port>\n       ulcp_hw_validate phase-b <port> <dev-key>\n       ulcp_hw_validate rf-peer <peer-port> <base-counter>\n       ulcp_hw_validate rf-dev-multicast <peer-port> <channel-key> <base-counter> [count]\n       ulcp_hw_validate rf-dev-unicast <peer-port> <dev-key> <base-counter> [count] [ack|silence]\n       ulcp_hw_validate rf-advert-request <peer-port> <dev-key> <counter> [nonce-hex]\n       ulcp_hw_validate phase-e <port> <count> <dropped> <acked>\n       ulcp_hw_validate info <port> [expected-host-key]\n       ulcp_hw_validate info-ble <selector> [expected-host-key]\n       ulcp_hw_validate ble-sync <selector>\n       ulcp_hw_validate hold <port> [seconds]\n       ulcp_hw_validate probe-restore <port>\n       ulcp_hw_validate battery <port>"
            );
            std::process::exit(2);
        }
    }
