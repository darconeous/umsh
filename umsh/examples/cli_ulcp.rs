...
//!     [--peer <b58key>[:alias]] ...
//!
//!   cargo run --example cli_ulcp --features cli,serial-radio -- \
...
//! device firmware also defaults to): --freq-khz, --bw-hz, --sf, --cr.
//!
    ...
    match cfg.transport.clone() {
        Transport::Ble(selector) => {
            #[cfg(feature = "ble-radio")]
            {
                ...
                let mut radio =
                    UlcpDevice::open_ble(selector.as_deref(), cfg.radio_config()).await?;
...
/// so a provisioned device's host-domain receive filters must not gate
/// live delivery: enable the session-scoped promiscuous mode (it
/// reverts on detach and touches no provisioning). devices predating the
/// property refuse the set; delivery then follows their filtering.
#[cfg(any(feature = "serial-radio", feature = "ble-radio"))]
async fn prepare_radio<L: FrameLink>(
    ...
    {
        Ok(_) => {}
        Err(UlcpError::Status(status)) => eprintln!(
...
impl Config {
    #[cfg(any(feature = "serial-radio", feature = "ble-radio"))]
    fn radio_config(&self) -> UlcpDeviceConfig {
        let mut config = UlcpDeviceConfig::new(self.freq_khz, self.bw_hz, self.sf, self.cr);
...
fn parse_args(args: Vec<String>) -> Result<Config, Box<dyn std::error::Error>> {
    ...
    // MeshCore-US bringup profile, matching the device firmware defaults.
