//! Smoke-test a ULCP radio device over serial or BLE.
//!
//! Attaches to the device (reset, version gate, RF configuration, PHY
//! enable), prints its identity, optionally transmits one test frame,
//! then prints every frame heard on air.
//!
//! ```sh
//! cargo run --example ulcp_probe --features serial-radio -- \
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let first = args.next().ok_or(
        "usage: ulcp_probe <serial-port> [--tx|--check] | --ble [selector] [--tx|--check]",
    )?;
    ...
    if first == "--ble" {
        ...
        #[cfg(feature = "ble-radio")]
        {
            ...
            let radio = UlcpDevice::open_ble(selector, config).await?;
    ...
    #[cfg(feature = "serial-radio")]
    {
        ...
        let radio = UlcpDevice::open_serial(&first, 115_200, config).await?;
        return run_probe(radio, do_tx, check_only, pin_update).await;
    }
...
async fn run_probe<L: FrameLink>(
    ...
    println!("device version: {}", radio.dev_version());
    ...
    if do_tx {
        radio
            .transmit(b"UMSH ULCP probe", TxOptions::default())
            .await
            .map_err(|error| format!("transmit failed: {error:?}"))?;
        println!("transmitted 20-byte test frame");

        // Duty only accumulates when the device's radio confirms an on-air
        // transmission, so a non-zero readback proves the TX completed.
        let duty = radio
            .get_prop(umsh::ulcp_wire::ids::prop::PHY_DUTY_NOW)
            .await?;
        if let [lo, hi, ..] = duty[..] {
