...
reset, or hold-boot-while-plugging.
...
- `make flash-t1000e-console` (UF2 drive `/Volumes/T1000-E`). **T1000-E has exactly two DFU entry paths: hold the user button while cycling USB power twice, or enter from software. There is no hold-the-button-while-plugging-in gesture** — do not describe one. The button path lands in the **UF2** bootloader, so flash by copy; serial DFU cannot talk to it and fails with "No data received on serial port". `make flash-t1000e-console-serial` is only for a board already sitting in serial DFU (override `DFU_SERIAL_PORT=/dev/tty.usbmodemN`)
