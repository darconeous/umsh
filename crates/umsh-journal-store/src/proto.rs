...
use crate::record::{self, CommitError, RecordWriter};
...
pub async fn write_record<W: RecordWriter>(
    ...
    crate::record::write_committed_record(writer, target, &bytes).await
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::record::{PAGE_SIZE, PageEraser};
    use core::future::Future;
    use core::task::{Context, Poll, Waker};

    /// Test-only journal addresses (production addresses are the
    /// firmware's memory-map decision).
    const PAGE0: u32 = 0x000E_8000;
    ...
    fn uncommitted_corrupt_and_oversize_records_are_ignored() {
        ...
        for (offset, bad) in [(9u32, (MAX_PAYLOAD as u16 + 1).to_le_bytes()), (8, [2, 0])] {
            ...
            let crc = crate::record::crc32(&bytes[..CRC_OFFSET]);
            bytes[CRC_OFFSET..COMMIT_OFFSET].copy_from_slice(&crc.to_le_bytes());
            let mut flash = MockFlash::new();
            block_on(RecordWriter::write_record(
                &mut flash,
