...
//! of [`sequential-storage`](https://docs.rs/sequential-storage).
//!
...
/// Errors surfaced by this module, generic over the flash driver's own
/// error type.
#[derive(Debug)]
pub enum Error<E> {
...
impl<E> From<sequential_storage::Error<E>> for Error<E> {
    fn from(err: sequential_storage::Error<E>) -> Self {
        Self::Storage(err)
    }
}
...
/// `Mac` platform.
pub struct FlashStore<F, M>
where
    F: MultiwriteNorFlash,
    M: RawMutex,
{
    map: Mutex<M, MapStorage<StoreKey, F, NoCache>>,
}
    ...
    M: RawMutex,
{
    ...
    async fn store_bytes(&self, key: &[u8], value: &[u8]) -> Result<(), Error<F::Error>> {
        let store_key = make_key(key)?;
        let mut scratch = [0u8; SCRATCH_LEN];
        let mut guard = self.map.lock().await;
    ...
    async fn delete_bytes(&self, key: &[u8]) -> Result<(), Error<F::Error>> {
        let store_key = make_key(key)?;
        let mut scratch = [0u8; SCRATCH_LEN];
        let mut guard = self.map.lock().await;
...
fn make_key<E>(bytes: &[u8]) -> Result<StoreKey, Error<E>> {
...
fn make_peer_key<E>(pk: &[u8; 32]) -> Result<StoreKey, Error<E>> {
    ...
    M: RawMutex,
{
    ...
    async fn read_peer_record(
        &self,
        pk: &[u8; 32],
    ) -> Result<Option<([u8; MAX_PEER_RECORD_LEN], usize)>, Error<F::Error>> {
    ...
    async fn load_peer_alias(
        &self,
        pk: &[u8; 32],
    ) -> Result<Option<heapless::String<MAX_ALIAS_LEN>>, Error<F::Error>> {
...
fn make_channel_key<E>(name: &[u8]) -> Result<StoreKey, Error<E>> {
    ...
    M: RawMutex,
{
    ...
    async fn load_channel_key(&self, name: &[u8]) -> Result<Option<[u8; 32]>, Error<F::Error>> {
    ...
    pub async fn delete_channel_entry(&self, name: &[u8]) -> Result<(), Error<F::Error>> {
        // Best-effort delete of the individual record.
        if let Ok(ch_key) = make_channel_key::<F::Error>(name) {
...
/// [`FlashStore`].
pub struct ChannelView<F, M>
where
    F: MultiwriteNorFlash + 'static,
    M: RawMutex + 'static,
{
    storage: &'static FlashStore<F, M>,
}

impl<F, M> ChannelView<F, M>
where
    F: MultiwriteNorFlash + 'static,
    M: RawMutex + 'static,
{
...
impl<F, M> umsh_hal::ChannelStore for ChannelView<F, M>
where
    F: MultiwriteNorFlash + 'static,
    M: RawMutex + 'static,
{
...
impl<F, M> umsh_hal::PeerStore for PeerView<F, M>
where
    F: MultiwriteNorFlash + 'static,
    M: RawMutex + 'static,
{
...
impl<F, M> umsh_hal::KeyValueStore for KeyValueView<F, M>
where
    F: MultiwriteNorFlash + 'static,
    M: RawMutex + 'static,
{
...
impl<F, M> umsh_hal::CounterStore for CounterView<F, M>
where
    F: MultiwriteNorFlash + 'static,
    M: RawMutex + 'static,
{
