        ...
        match expected {
            ...
            ExpectedResponse::ManagementSave => {
                if response.property_id != prop::LAST_STATUS
                    || response.command != Cmd::PropIs as u8
                {
                    // The save's outcome is unknown, and the completion
                    // says so by carrying no status.
                    state.note_mismatch("save device configuration".to_owned(), &response);
                } else {
                    let status_code = inspect_ulcp_status(response.value.clone())?;
...
mod tests {
    ...
    fn a_device_channel_echo_in_an_unknown_form_fails_the_operation_and_reads_back() {
        ...
        let key = vec![0xB7; 32];
        let id = crate::derive_channel_id(key.clone()).unwrap();
        let insert = session.insert_device_channel_key(key).unwrap();
        let request = Frame::parse(&insert.outbound_frames[0]).unwrap();

        // Firmware that reports channels in a form this session does not
        // predict—a wider identifier, say—confirms an item that is not the
        // one this session derived.
        let echoed = vec![0xEE; 16];
        let confirmed = session
            .consume(inserted_response(
                request.header.tid(),
                prop::DEV_CHANNEL_KEYS,
