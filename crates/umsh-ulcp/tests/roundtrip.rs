//! End-to-end wire round-trips: ULCP frame inside HDLC-Lite.

use umsh_ulcp::frame::{self, Cmd, Frame, PropPayload, StreamPayload};
use umsh_ulcp::hdlc;
use umsh_ulcp::ids::{prop, stream};
use umsh_ulcp::meta::RxMeta;
use umsh_ulcp::{Status, TxMeta};
...
fn raw_frame_tx_and_rx_over_hdlc() {
    // Host -> device: transmit request with TX metadata.
    ...
    // Device -> host: completion notification.
    ...
    // Device -> host: received packet with RX metadata.
    let rx_meta = RxMeta {
        rssi_dbm: Some(-103),
        lqi: None,
        snr_cb: Some(-78),
